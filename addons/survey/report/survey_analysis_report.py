...
class survey_analysis(report_rml):
    def create(self, cr, uid, ids, datas, context):
        ...
        ids2=[]
        for surv in surv_obj.browse(cr, uid, ids):
            if surv.date_open >= datas['form']['date_start'] and surv.date_open <= datas['form']['date_end'] :
                ids2.append(surv.id)
        for survey in surv_obj.browse(cr, uid, ids2):
            rml += """<story>
                    <para style="Title">RAPPORT SATISFACTION CLIENT</para>
                    ...
                    <blockTable colWidths="280.0,100.0,120.0" style="Table_heading">
                      <tr>
                        <td>
                          <para style="terp_tblheader_General_Centre">Titre </para>
                        </td>
                        <td>
                          <para style="terp_tblheader_General_Centre">Nombre de clients</para>
                        </td>
                        <td>
                          <para style="terp_tblheader_General_Centre">Reponses completes</para>
                        </td>
                      </tr>
                      </blockTable>
            ...
            for page in survey.page_ids:
                rml += """ <blockTable colWidths="500" style="Table4">
                              <tr>
                                <td><para style="page">""" + to_xml(tools.ustr(page.title)) + """</para></td>
                              </tr>
                           </blockTable>"""
                for que in page.question_ids:
                    ...
                    if que.type in ['matrix_of_choices_only_one_ans','matrix_of_choices_only_multi_ans']:
                        ...
                        rml += """<td><para style="response-bold">Nombres Reponses</para></td>
                                </tr>"""
                        ...
                        for ans in que.answer_choice_ids:
                            ...
                            for mat_col in range(1, len(matrix_ans)):
                                ...
                                #if percantage:
                                progress = percantage * 1.8 / 100
                                #geerish
                                rml += """<td color="#FFF435">
                                
                                <illustration>
                                <stroke color="lightslategray"/>
                                   <rect x="0.0cm" y="-0.45cm" width="2 cm" height="0.5cm" fill="no" stroke="yes"  round="0.1cm"/>
                                   """
                                rml +=""""""
                                if percantage>0:
                                    rml += """<fill color="lightsteelblue"/>
                                       <rect x="0.1cm" y="-0.35cm"  width='""" + tools.ustr(str(float(progress)) +'cm') + """' height="0.3cm" fill="yes" stroke="no"  round="0.1cm"/>
                                       <fill color="black"/>
                                       <setFont name="Helvetica" size="9"/>
                                        <drawString x="0.4cm" y="-0.31cm">"""  + tools.ustr(percantage) + """%</drawString>
                        ...
                        rml += """ <tr>
                             ...
                             <td> <para style="terp_default_Center_heading">Pourcentage Reponse</para></td>
                             <td> <para style="response-bold">Nombres Reponses</para></td>
                         </tr>"""
...
+                                       group by sra.answer_id,sqc.rating_weight", (ans.id,matrix_ans[mat_col][1]))
                        ...
                        for column in que.column_heading_ids:
                            ...
                            rml += """<td><para style="response-bold">Nombres Reponses</para></td></tr>"""
                    ...
                    rml +="""<blockTable colWidths="300,100,100.0" style="Table3">
                        <tr>
                              ...
                              <td><para style="Standard1">Repondu</para></td>
                              <td><para style="Standard1">""" + tools.ustr(que.tot_resp) + """</para></td>
                        </tr>
                        <tr>
                            <td><para style="Standard1"></para></td>
                            <td><para style="Standard1">Non-Repondu</para></td>
