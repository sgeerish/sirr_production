...
def _invoice_merge(self, cr, uid, data, context):
	...
	if len(vals):
	    inv_obj.write(cr, uid,[invs[0]['id'],], vals, context)
	...
	# Handle sales orders:
	sql_req = "UPDATE sale_order_invoice_rel SET invoice_id = %s WHERE invoice_id = ANY(%s); "
        cr.execute(sql_req, (invs[0]['id'], inv_ids))
