...
class hr_employee(osv.osv):
    ...
    def _state(self, cr, uid, ids, name, args, context=None):
        ...
        for id in ids:
            ...
            holidays=holiday_obj.search(cr,uid,[('employee_id','=',id),('type','=','remove'),('date_from','<=',now),('date_to','>=',now),('state','=','validate')])
            print id
            print holidays
            print now
            ...
            atts=att_obj.search(cr,uid,[('employee_id','=',id)])
            atts=att_obj.browse(cr,uid,atts)
            if len(atts)>0:
                if(atts[0].name[:10]!=now[:10]) and result[id]!='holiday':
                    result[id]='absent'
        return result

    _columns = {
       'state': fields.function(_state, method=True, type='selection', selection=[('absent', 'Absent'), ('present', 'Present'),('holiday','Holiday')], string='Attendance'),
    }
