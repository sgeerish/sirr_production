...
#                
    ...
    def onchange_partner_id_simple(self, cr, uid, ids, partner_id, journal_id, price, currency_id, ttype, date,invoice_id, context=None):
        """price
        Returns a dict that contains new values and context
        ...
        if not context.get('move_line_ids', False):
            if invoice_id:
                ids= move_line_pool.search(cr, uid, [('move_id','=',invoice.move_id.id),('state','=','valid'), ('account_id.type', '=', account_type), ('reconcile_id', '=', False), ('partner_id', '=', partner_id)], context=context)
            else:
                ids = move_line_pool.search(cr, uid, [('state','=','valid'), ('account_id.type', '=', account_type), ('reconcile_id', '=', False), ('partner_id', '=', partner_id)], context=context)
        else:
            ids = context['move_line_ids']
...
#            reconcile_pool.unlink(cr, uid, recs)
    ...
    def action_move_line_create(self, cr, uid, ids, context=None):
        ...
        for inv in self.browse(cr, uid, ids, context=context):
            if not inv.partner_id.x_maturity:
                if inv.type in ('sale', 'receipt'):
                    flag_due=False
                    for iv_line in inv.line_cr_ids:
                        if iv_line.amount>0:
                            if inv.banking_date>iv_line.date_due:
                                flag_due=True
                    if flag_due:
                        raise osv.except_osv(_('Erreur !'), _('Date Echeance incompatible!'))
            ...
            if inv.x_sale_id:
                prod_lines=prod_obj.search(cr,uid,[('x_sale_order','=',inv.x_sale_id.id)])
                prod_lines=prod_obj.browse(cr,uid,prod_lines)
                for prod in prod_lines:
                    prod_obj.write(cr,uid,prod.id,{'x_account_voucher':inv.id})
            if inv.number:
                name = inv.number
		...
		'ref':inv.ref,
            ...
            for line in inv.line_ids:
                ...
                move_line = {
                    ...
                    'debit': 0.0,
                    'date': inv.date,
		    'date_maturity':inv.instrument_date or inv.date,
                }
                if amount < 0:
                    amount = -amount
                    if line.type == 'dr':
            ...
            if not currency_pool.is_zero(cr, uid, inv.currency_id, line_total):
                ...
                move_line = {
                    'name': name,
                    'account_id': inv.voucher_type_id and inv.voucher_type_id.account_id.id or account_id,
                    'move_id': move_id,
                    'partner_id': inv.partner_id.id,
                    'date': inv.date,
