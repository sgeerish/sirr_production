...
class sale_order(osv.osv):
    ...
    def _amount_line_tax(self, cr, uid, line, context=None):
        ...
        for c in self.pool.get('account.tax').compute_all(cr, uid, line.tax_id, line.price_unit * (1-(line.discount or 0.0)/100.0), line.product_uom_qty*th_weight, line.order_id.partner_invoice_id.id, line.product_id, line.order_id.partner_id)['taxes']:
    ...
    def _invoiced(self, cursor, user, ids, name, arg, context=None):
        #changepricelist before confirmation
        cur_obj = self.pool.get('res.currency')
        obj_curr = self.browse(cursor, user, ids, context)[0]
        ...
        if sale_obj['state'] == 'draft':
            ...
            for line in sale_obj.order_line:
                vals = order_line_obj.product_id_change(cursor, user, line.id, pricelist_id.id, line.product_id.id ,qty=line.product_uom_qty,uom=line.product_uom.id,uos=line.product_uos.id, partner_id=partner_id, date_order=date_order)
                if vals.get('value',False):
                    if 'price_unit' in vals['value'].keys():
                        order_line_obj.write(cursor, user, line.id, {'price_unit': vals['value']['price_unit']},context=context)
#        else:
...
###   
  ...
  - The 'Invoice From The Picking' choice is used to create an invoice during the picking process."""),
        ...
        'order_line': fields.one2many('sale.order.line', 'order_id', 'Order Lines', readonly=False, states={'draft': [('readonly', False)]}),
    ...
    _defaults = {
        ...
        'order_policy': 'manual',
        'state': 'draft',
#        'shop_id': lambda obj, cr, uid, context: obj.pool.get('res.users').browse(cr,uid,uid).shop,
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        products=[]
        limit_tariff=params.limit_tariff
        limit_free=params.limit_free
        for so in so_obj.browse(cr,uid,ids):
            amount_total=0
            if so.state=='draft':
                # if not so.delivery_product_id:
                for sl in so.order_line:
                    if sl.x_livraison=='livraison':
                        amount_total+=sl.price_subtotal
                        products.append(sl.product_id.id)
                if vals.get('delivery_zone',False):
                    if products in params.special_products:
                        #to add a field to cater for the free product limit
                        limit_free=1400000
                        if amount_total<limit_free:
                            zone=self.pool.get('delivery.zone').browse(cr,uid,vals['delivery_zone'])
                            if amount_total<limit_tariff:
                                delivery_product=zone.limit1
                            else:
                                delivery_product=zone.limit2
                        else:
                            delivery_product=params.delivery_product_id_free
                    else:
                        if so.partner_id in params.chargeable_partner_ids:
                            if amount_total<limit_free:
                                zone=self.pool.get('delivery.zone').browse(cr,uid,vals['delivery_zone'])
                                if amount_total<limit_tariff:
                                    delivery_product=zone.limit1
                            ...
                            delivery_product=params.delivery_product_id_free
                    vals.update({'delivery_product_id' : delivery_product.id,'delivery_amount':delivery_product.lst_price,'delivery_product':True})
    ...
    def _make_invoice(self, cr, uid, order, lines, context=None):
        ...
        inv = {
            ...
            'name_divers': order.name_divers,
            'address_divers': order.address_divers,
            'tel_divers': order.tel_divers,
            'journal_id': journal_ids[0],
            'address_invoice_id': order.partner_invoice_id.id,
            'address_contact_id': order.partner_order_id.id,
            'invoice_line': [(6, 0, lines)],
            'currency_id': order.pricelist_id.currency_id.id,
            'comment': order.x_note or order.note,
    ...
    def action_wait(self, cr, uid, ids, *args):
        for o in self.browse(cr, uid, ids):
            if (o.order_policy == 'manual'):
                self.write(cr, uid, [o.id], {'state': 'manual','validate':uid, 'date_confirm': time.strftime('%Y-%m-%d')})
            else:
                self.write(cr, uid, [o.id], {'state': 'progress','validate':uid, 'date_confirm': time.strftime('%Y-%m-%d')})
    ...
    def action_ship_create(self, cr, uid, ids, *args):
        ...
        for order in self.browse(cr, uid, ids, context={}):
            ...
            pickings=[]
            for line in order.order_line:
                if line.location.id not in locations:
                    locations.append(line.location.id)
            ...
            if picking_id:
                for p in pickings:
                    wf_service.trg_validate(uid, 'stock.picking',p, 'button_confirm', cr)
...
class sale_order_line(osv.osv):
    ...
    def _amount_line(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if (line.th_weight>0):
                qty=line.product_uom_qty*line.th_weight
            else:
                qty=line.product_uom_qty
            taxes = tax_obj.compute_all(cr, uid, line.tax_id, price, qty, line.order_id.partner_invoice_id.id, line.product_id, line.order_id.partner_id)
    ...
    _columns = {
        ...
        'discount': fields.float('Discount (%)', digits=(16, 7), readonly=True, states={'draft': [('readonly', False)]}),
    ...
    def invoice_line_create(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            if not line.invoiced:
                ...
                if line.order_id.user_id.shop.x_journal.default_credit_account_id:
                    a=line.order_id.user_id.shop.x_journal.default_credit_account_id.id
                ...
                if uosqty:
                    if line.product_id:
#                        pu=round(line.product_id.lst_price * line.product_uom_qty / uosqty,
#                            self.pool.get('decimal.precision').precision_get(cr, uid, 'Sale Price'))
                        pu=round(line.price_unit * line.product_uom_qty / uosqty,
                            self.pool.get('decimal.precision').precision_get(cr, uid, 'Sale Price'))                            
			pu2=self.pool.get('product.pricelist').price_get(cr, uid, [line.order_id.pricelist_id.id],
		    ...
		    'x_promo':line.x_promo,
                    ...
                    'x_prodlot_id':line.x_prodlot_id.id or False,
                    'landing_cost': 1,
                    'actual_sale_price':1,
                    'nouv_marge':1,
                    'marge_achat':1,
        ...
        if line.order_id.delivery_product_id:
	    del_id = self.pool.get('account.invoice.line').create(cr, uid, {
                    'name': 'Frais de Livraison',
                    'origin': line.order_id.name,
                    'location':line.location.id,                    
                    'account_id': a,
                    'price_unit': line.order_id.delivery_product_id.lst_price,
                    'quantity': 1,
                    'discount': 0,
                    'uos_id': line.order_id.delivery_product_id.uom_id.id or False,
                    'product_id': line.order_id.delivery_product_id.id or False,
                    'account_analytic_id':False,                     
                    'invoice_line_tax_id': [(6, 0, [x.id for x in line.order_id.delivery_product_id.taxes_id])],
                })
            create_ids.append(del_id)
    ...
    def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
        ...
        location=user_obj.browse(cr,uid,uid).location
        domain = {}
	if partner_id:
        ...
        result = {}
        result['discount']=0
	if self.browse(cr,uid,ids):
            if isinstance(ids, (int, long)):
                ids = [ids]
            location=self.browse(cr,uid,ids)[0].location
        else:
            location=self.pool.get('res.users').browse(cr,uid,uid).location            
            #print 'no browse'

        if not isinstance(location,(int,long,tuple,list)):
            location=self.pool.get('res.users').browse(cr,uid,uid).location
        if location.id=='':
            location=self.pool.get('res.users').browse(cr,uid,uid).location	
        actual_location=obj_location.browse(cr,uid,location.id)
        location=location.id
        actual_location=actual_location.name        
        context['location']=location        
        product_obj = product_obj.browse(cr, uid, product, context=context)
        actual_available=product_obj.virtual_available
        #print '__'
        #print context['location']
        #print actual_available
        #print '__'
    ...
    #            qty = qty - qty % q + q
        ...
        for l in obj_location.browse(cr,uid,locations):
            ...
            prod=product_obj_new.browse(cr, uid, product, context=context)
            if (prod.virtual_available>0):
                if (stock==''):
                    stock+='\n\nStock Disponible pour la Vente : \n'
                stock+='%s \t:\t%d\n' %(l.name,prod.virtual_available)
        if (product_obj.type=='product') and (int(actual_available) < int(qty) ) :
            # print self.order_id print sale_order.browse(cr,uid,self.order_id).name result['product_uom_qty'] = max(0,product_obj.qty_available) result['product_uos_qty'] = 
            #max(0,product_obj.qty_available)
            # print 'setting stock_ok False' self.write(cr,uid,ids,{'stock_ok':False}) print self.stock_ok
            # print 'ACT:'%actual_available
            # print 'QTY'%qty
            if int(actual_available)>=int(qty):            
                result.update({'stock_ok':True})
                result['stock_ok']=True
            else:
                result.update({'stock_ok':False})
                result['stock_ok']=False            

            warning = {
                'title': _('Pas assez de stock dans le depot %s!') % actual_location,
                'message': _('Vous tentez de vendre %.2f %s mais seulement %.2f %s disponible dans le depot %s!%s') %
                    (qty, uom2 and uom2.name or product_obj.uom_id.name,
                     max(0,actual_available), product_obj.uom_id.name,
                     actual_location,
    ...
    #def product_discount_change(self, cr, uid, ids, discount):
    #    warning={}
    #    if discount>10:
    #        warning = {
