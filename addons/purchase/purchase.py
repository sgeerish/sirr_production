...
class purchase_order(osv.osv):
    ...
    STATE_SELECTION = [
        ('draft', 'Brouillon'),
        ('start', 'Nouvelle Requete'),
        ('wait', 'Waiting'),
        ('confirmed', 'Validé par Direction Générale'),
        ('approved', 'Approved'),
        ('except_picking', 'Shipping Exception'),
        ('except_invoice', 'Invoice Exception'),
        ('done', 'Done'),
        ('cancel', 'Cancelled'),
        ('budget_valid','Validé par Dir. Com.'),
        ('finance_valid','Validé par DAF')
    ]
    ...
    def purchase_budget(self, cr, uid, ids, context=None):
		self.write(cr, uid, ids, {'state': 'budget_valid', 'sb':uid, 'date_sb': time.strftime('%Y-%m-%d')})
		return True
	
    def purchase_daf(self, cr, uid, ids, context=None):
        self.write(cr, uid, ids, {'state': 'finance_valid', 'daf':uid,'date_daf': time.strftime('%Y-%m-%d')})
        return True

    def purchase_start(self, cr, uid, ids, context=None):
        for po in self.browse(cr,uid,ids):
            if po.check_min_stock(cr,uid,ids):
...
#        current_name = self.name_get(cr, uid, ids)[0][1]
    ...
    def wkf_warn_buyer(self, cr, uid, ids):
        self.write(cr, uid, ids, {'state' : 'wait', 'dg' : uid})
    ...
    def action_invoice_create(self, cr, uid, ids, *args):
        ...
        for o in self.browse(cr, uid, ids):
            ...
            res = inv_id
            if o.client_id:
                o.action_customer_invoice_create(cr,uid,ids)
        return res
            ...
            for order_line in order.order_line:
                ...
                if order_line.product_id.product_tmpl_id.type in ('product', 'consu'):
                    ...
                    move = self.pool.get('stock.move').create(cr, uid, {
                        ...
                        'x_discount':order_line.discount
                    })
...
class purchase_order_line(osv.osv):
    ...
    _columns = {
        ...
        'last_price': fields.float('Last Price', digits_compute= dp.get_precision('Purchase Price')),
    ...
    def product_id_change(self, cr, uid, ids, pricelist, product, qty, uom,
        ...
        if not product:
            return {'value': {'price_unit': price_unit or 0.0,'last_price': price_unit or 0.0, 'name': name or '',
        ...
        res.update({'value': {'price_unit': price,'last_price': price, 'name': prod_name,
...
class procurement_order(osv.osv):
    ...
    def action_po_assign(self, cr, uid, ids, context=None):
        ...
        @return: True
        """
        message = _("Bon de Commande Lancer.")
	for id in ids:
        	self.log(cr, uid,id, message)
