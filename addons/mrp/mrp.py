...
class mrp_routing_workcenter(osv.osv):
    ...
    def onchange_product_id(self, cr, uid, ids, workcenter_id, name, context=None):
        ...
        print workcenter_id    
        if workcenter_id:
            prod = self.pool.get('mrp.workcenter').browse(cr, uid, workcenter_id, context=context)
            print prod.name
            v = {}
            if not name:
                v['name'] = '['+prod.code+'] '+prod.name
            return {'value': v}
        return {}
...
class mrp_production(osv.osv):
    ...
    _columns = {
        ...
        'product_id': fields.many2one('product.product', 'Product', required=True, ),
        'product_qty': fields.float('Product Qty', required=True, states={'draft':[('readonly',False)]}, readonly=True, digits=(16,3)),
    ...
    def action_compute(self, cr, uid, ids, properties=[]):
        ...
        for production in self.browse(cr, uid, ids):
            ...
            if production.th_weight:
                factor=factor*production.th_weight
