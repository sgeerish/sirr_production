...
class sale_order_line(osv.osv):
	...
	#print location.id
	#for l in user_location:
	#	location=l.location
       # if self.browse(cr,uid,ids):
#	     print self.browse(cr,uid,ids)[0]
...
#            qty = qty - qty % q + q
	...
	obj_location=self.pool.get('stock.location')
        ...
        stock='';
	actual_location=obj_location.browse(cr,uid,location)
	...
	actual_available=product_obj.virtual_available    
	if (product_obj.type=='product') and (actual_available * uom2.factor < qty * product_obj.uom_id.factor) \
