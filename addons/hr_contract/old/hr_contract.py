...
class hr_employee(osv.osv):
    ...
    _columns = {
        ...
        'contract_id': fields.one2many('hr.contract', 'employee_id', 'Contracts'),
    }
hr_employee()
...
class hr_contract_wage_type2(osv.osv):
    _name = 'hr.contract.wage.type2'
    _description = 'Wage Type2'
    _columns = {
        'name' : fields.char('Wage Type Name', size=50, required=True, select=True),
        'period_id' : fields.many2one('hr.contract.wage.type.period', 'Wage Period', required=True),
        'type' : fields.selection([('gross','Gross'), ('net','Net')], 'Type', required=True),
...
class hr_contract(osv.osv):
    ...
    _columns = {
        ...
        'trial_date_start': fields.date('Start Date', required=True),
        'trial_date_end': fields.date('Start Date', required=True),
        'date_end': fields.date('End Date'),
        'working_hours': fields.many2one('resource.calendar','Working Schedule'),
        'wage_type_id': fields.many2one('hr.contract.wage.type2', 'Wage Type'),
