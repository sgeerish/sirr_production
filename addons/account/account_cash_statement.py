...
class account_cash_statement(osv.osv):
    ...
    def _get_sum_entry_encoding(self, cr, uid, ids, name, arg, context=None):
        ...
        mids=[]
        for voucher in voucher_obj.search(cr,uid,[('state','=','posted'),('date','=',date),('journal_id','=',journal.id)]):
            mids.append(voucher)
        total=0
        for voucher in voucher_obj.browse(cr,uid,mids):
	    #print 'amount',voucher.amount
    ...
    def _end_balance(self, cursor, user, ids, name, attr, context=None):
        ...
        for r in res:
            res[r] = res[r]
        #res[r]=statement.total_entry_encoding+statement.balance_start
        return res
    ...
    def _get_cash_open_box_lines(self, cr, uid, context=None):
        res = []
        curr = [10000, 5000, 2000, 1000, 500, 200, 100, -1]
        for rs in curr:
            dct = {
                'pieces': rs,
                'number': 0
            ...
            res.append(dct)
        journal_ids = self.pool.get('account.journal').search(cr, uid, [('type', '=', 'cash'),('user_id','=',uid)], context=context)
        if journal_ids:
    ...
    def _get_cash_open_box_lines2(self, cr, uid,journal, context=None):
        res = []
        curr = [10000, 5000, 2000, 1000, 500, 200, 100, -1]
        for rs in curr:
            dct = {
            ...
            res.append(dct)
#        journal_ids = self.pool.get('account.journal').search(cr, uid, [('journal_id','=',journal),('type', '=', 'cash')], context=context)
#        if journal_ids:
        results = self.search(cr, uid, [('journal_id', '=', journal),('state', '=', 'confirm')], context=context)
        if results:
            cash_st = self.browse(cr, uid, results, context=context)[0]
            print cash_st.name
            for cash_line in cash_st.ending_details_ids:
    ...
    _columns = {
        ...
        'balance_end': fields.function(_end_balance, method=True, string='Balance', help="Closing balance based on Starting Balance and Cash Transactions"),
...
#	return super(account_cash_statement, self).onchange_journal_id(cr, uid, statement_id, journal_id, context=context)
    ...
    def onchange_journal_id(self, cr, uid, statement_id, journal_id,context=None):
        ...
        res = {}
        balance_start = 0.0
        #starting_details=self._get_cash_open_box_lines2(cr,uid,journal_id)
        #res.update({'starting_details_ids':starting_details})
        if not journal_id:
            res.update({
                'balance_start': balance_start,
            })
	res.update(super(account_cash_statement, self).onchange_journal_id(cr, uid, statement_id, journal_id, context=context))
	#print res['value']
    ...
    def _equal_balance(self, cr, uid, cash_id, context=None):
        ...
        balance_end=statement.total_entry_encoding+statement.balance_end
        self.write(cr, uid, [cash_id], {'balance_end_real': balance_end})
        if int(statement.balance_end_cash)!=int(balance_end):
            return False
        return True
    ...
    def button_open(self, cr, uid, ids, context=None):
        ...
        for statement in statement_pool.browse(cr, uid, ids, context=context):
            ...
            if statement.name and statement.name == '/':
                ...
                number = self.pool.get('res.users').browse(cr,uid,uid).name+' '+number
		vals.update({
                    'name': number
                })
    ...
    def button_confirm_cash(self, cr, uid, ids, context=None):
		...
		x=self.create_move_from_st_line2(cr, uid, st_line.id, company_currency_id, st_line_number, context)
                #print 'statement %s ccy % s line % s %d' % (st_line.id,company_currency_id,st_line_number,count)
            #print count
