...
class account_invoice(osv.osv):
    ...
    _columns = {
		...
		#'regle': fields.function(_regle, method=True, string='Reglé',
        #    fnct_search=_regle_search, type='boolean'),
    ...
    def test_paid(self, cr, uid, ids, *args):
        ...
        #    if (inv.payment_term.name!='Comptant'):
        #        return True
        if not res:
            return False
        ok = True
...
#        return "%s-%s-%s-%s-%s"%(
    ...
    def action_move_create(self, cr, uid, ids, *args):
        ...
        for inv in self.browse(cr, uid, ids):
            ...
            move = {
                ...
                'date': date,
                'type': entry_type,
                'narration':inv.comment and inv.comment+inv.origin or inv.origin
            }
    ...
    def action_cancel(self, cr, uid, ids, *args):
        ...
        stock_move_obj = self.pool.get('stock.picking')  
        ...
        for i in invoices:
            ...
            invoice=invoice_obj.search(cr,uid,[('number','=',inv.number)])
            invoice=invoice_obj.browse(cr,uid,invoice)
            stock_available=False
            if not inv.x_so:
                if inv.number:
                    stock=stock_move_obj.search(cr, uid, [('origin','=',inv.number)])
                    stock=stock_move_obj.browse(cr,uid,stock)
                    for s in stock:
                        if (s.state!='cancel'):
                            stock_available=True
            if stock_available:
                raise osv.except_osv(_('Error !'), _('Veuillez Annuler Les Bons de Livraisons!'))
...
#        return map(lambda x: (0,0,x), lines)
    ...
    def refund(self, cr, uid, ids, date=None, period_id=None, description=None, journal_id=None):
        ...
        for invoice in invoices:
            ...
            invoice_lines = obj_invoice_line.browse(cr, uid, invoice['invoice_line'])
		...
		inv_list.append(obj_invoice_line.copy_data(cr, uid, inv_line['id']))
            ...
            # tax_lines = filter(lambda l: l['manual'], tax_lines) tax_lines = self._refund_cleanup_lines(cr, uid, tax_lines)
            tax_list = []
...
#                'tax_line': tax_list[0],
            ...
            if tax_list!=[]:
                invoice.update({'tax_line':tax_list[0]})
...
class account_invoice_line(osv.osv):
    def _amount_line(self, cr, uid, ids, prop, unknow_none, unknow_dict):
        ...
        for line in self.browse(cr, uid, ids):
            ...
            if line.invoice_id.conversion:
                ...
                price=((price*tonnage)/total_qty)
                print 'calculated_price',price
    ...
    def _price_unit_default(self, cr, uid, context=None):
        ...
        if 'check_total' in context:
		    ...
		    #if taxes:
		    #	if len(taxes)>0 and len(taxes[0]) >= 3 and taxes[0][2]:
                    if( taxes!=None):
    ...
    _columns = {
        ...
        'new_price': fields.float('New Price', digits_compute= dp.get_precision('Purchase Price')),
        ...
        'discount': fields.float('Discount (%)', digits=(14,6)),
...
class account_invoice_tax(osv.osv):
    ...
    def compute(self, cr, uid, invoice_id, context=None):
        ...
        for line in inv.invoice_line:
            ...
            for tax in tax_obj.compute_all(cr, uid, line.invoice_line_tax_id, (line.price_unit* (1-(line.discount or 0.0)/100.0)), line.quantity*th_weight, inv.address_invoice_id.id, line.product_id, inv.partner_id)['taxes']:
                val={}
