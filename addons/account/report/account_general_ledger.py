...
class general_ledger(report_sxw.rml_parse, common_report_header):
    ...
    def lines(self, account):
        ...
        sql = """
            ...
            p.ref  AS partner_name
            FROM account_move_line l
            JOIN account_move m on (l.move_id=m.id)
            LEFT JOIN account_move_reconcile c on (l.reconcile_id=c.id)
        ...
        if res_lines and self.init_balance:
            ...
            sql = """
                SELECT 0 AS lid, '' AS ldate, '' AS lcode, COALESCE(SUM(l.amount_currency),0.0) AS amount_currency, '' AS lref, 'Solde initial' AS lname, COALESCE(SUM(l.debit),0.0) AS debit, COALESCE(SUM(l.credit),0.0) AS credit, '' AS lperiod_id, '' AS lpartner_id,
