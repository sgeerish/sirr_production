...
class account_bank_statement(osv.osv):
    ...
    def button_dummy(self, cr, uid, ids, context=None):
        # print self._end_balance(cr,uid,ids)
        return self.write(cr, uid, ids, {}, context=context)

    def create_move_from_st_line_(self, cr, uid, st_line_id, company_currency_id, st_line_number, context=None):
        if context is None:
            context = {}
        res_currency_obj = self.pool.get('res.currency')
        ...
        # st_move=account_move_obj.browse(cr,uid,st_line.voucher_id.id)
        # st_move_line=st_move.line_id
        ...
        if st.currency.id <> company_currency_id:
            ...
            currency_id = st.currency.id
        move2=account_move_line_obj.create(cr, uid, {
            'name': st_line.name,
            'date': st_line.date,
            'ref': st_line.ref,
            'move_id': move_id,
        ...
        rec_list_ids=[]
        if st_line.move_line:
            rec_ids = [move_line_id, st_line.move_line.id]  
            rec_list_ids.append(rec_ids)
            for rec_ids in rec_list_ids:
                if len(rec_ids) >= 2:
                    account_move_line_obj.reconcile_partial(cr, uid, rec_ids)
    ...
    def button_confirm_bank(self, cr, uid, ids, context=None):
        ...
        for st in self.browse(cr, uid, ids, context=context):
            ...
            for st_line in st.line_ids:
                ...
                count+=1
                x=self.create_move_from_st_line_(cr, uid, st_line.id, company_currency_id, st_line_number, context)
            self.write(cr, uid, [st.id], {'name': st_number}, context=context)
            self.log(cr, uid, st.id, _('Statement %s is confirmed, journal items are created.') % (st_number,))
            done.append(st.id)
        return self.write(cr, uid, ids, {'state':'confirm'}, context=context)

    def button_confirm_bank_old(self, cr, uid, ids, context=None):
        done = []
        obj_seq = self.pool.get('ir.sequence')
        if context is None:
            context = {}
        ...
        for st in self.browse(cr, uid, ids, context=context):
            ...
            for line in st.move_line_ids:
                if line.state <> 'valid':
                    raise osv.except_osv(_('Error !'),
                            _('The account entries lines are not in valid state.%s - %s')%(line.state,line.name))
	    count=0
            for st_line in st.line_ids:
		count+=1
                if st_line.analytic_account_id:
