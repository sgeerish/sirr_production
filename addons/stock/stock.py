        ...
        for id in ids:
            ...
            for uom_id in products_by_uom.keys():
                ...
                for product_id in qty.keys():
                    ...
                    reads=product_obj.read(cr, uid, product.id, ['virtual_available'],context=ctx)
                    vqty=reads['virtual_available']
                    price = vqty * amount_unit
                    quantity_total += vqty
...
class stock_picking(osv.osv):
    ...
    def create(self, cr, user, vals, context=None):
        ...
        user_login=user_obj.browse(cr,user,user).login
        if user_login=='vero':
            user_login='sherbanu'
        if ('name' not in vals) or (vals.get('name')=='/'):
            seq_obj_name =  'stock.picking.' + vals['type']
            if vals['type']=='out' and user_login=='sherbanu':
                seq_obj_name =seq_obj_name+ '.' + user_login
                print seq_obj_name
    ...
    def action_done(self, cr, uid, ids, context=None):
        ...
        self.write(cr,uid,ids,{'x_user_id':uid})
        return True
    ...
    def _get_invoice_type(self, pick):
        ...
        if pick.invoice_state == '2binvoiced':
            ...
            if pick.type == 'out' and dest_usage == 'supplier':
                ...
                inv_type = 'out_invoice'
        #print 'SRC:'+src_usage
        #print 'DEST:'+dest_usage
        #print 'PICK:'+pick.type
        #print 'Inv:'+inv_type
        return inv_type
    def temp_invoice(self, cr, uid, ids, journal_id,
            group=False, type=None, context=None):
        ...
        for picking in self.browse(cr, uid, ids, context=context):
            ...
            #print self._get_invoice_type(picking)
            #print 'After GERT:'+inv_type
            ...
            'origin': (invoice.origin or '') + ', ' + (picking.name or '') + (picking.origin and (':' + picking.origin) or ''),
                ...
                invoice_vals = {
                    'name': (picking.name or '') + (picking.origin and (':' + picking.origin) or ''),
            ...
            for move_line in picking.move_lines:
                ...
                if inv_type in ('in_refund','out_invoice'):
                    loc=move_line.location_id.id
                else:
                    loc=move_line.location_dest_id.id
                ...
                invoice_line_id = invoice_line_obj.create(cr, uid, {
                    ...
                    'discount': move_line.x_discount,
                    'quantity': move_line.product_uos_qty or move_line.product_qty,
                    'invoice_line_tax_id': [(6, 0, tax_ids)],
                    'account_analytic_id': account_analytic_id,
                    'x_unit_discount':price_unit-((move_line.x_discount/100)*price_unit),
                    'x_move_id':move_line.id,
                    'location':loc
                }, context=context)
    ...
    def log_picking(self, cr, uid, ids, context=None):
        ...
        for pick in self.browse(cr, uid, ids, context=context):
            ...
            if pick.min_date:
                msg= _(' for the ')+ datetime.strptime(pick.min_date[:19], '%Y-%m-%d %H:%M:%S').strftime('%m/%d/%Y')
            state_list = {
...
class stock_move(osv.osv):
    ...
    def onchange_quantity(self, cr, uid, ids, product_id, product_qty,
                          product_uom, product_uos,location):
        ...
        c={}
        c['location']=location
        available=product_obj.browse(cr,uid,product_id,context=c).virtual_available
        if product_qty>available:
    ...
    def onchange_quantity_in(self, cr, uid, ids, product_id, product_qty,
    ...
    def onchange_product_id(self, cr, uid, ids, prod_id=False, loc_id=False,
        ...
        result = {
            ...
            'product_uos': uos_id,
            'product_qty': self.pool.get('stock.move').onchange_quantity(cr, uid, ids, prod_id, 1.00, product.uom_id.id, uos_id,loc_id)['value']['product_qty'],
        ...
        for picking, todo in self._chain_compute(cr, uid, moves, context=context).items():
            ...
            for move, (loc, dummy, delay, dummy, company_id, ptype) in todo:
                new_id = move_obj.copy(cr, uid, move.id, {
                    ...
                    'move_history_ids': [],
                    'date': (datetime.strptime(move.date[:19], '%Y-%m-%d %H:%M:%S') + relativedelta(days=delay or 0)).strftime('%Y-%m-%d'),
                    'move_history_ids2': []}
                )
                move_obj.write(cr, uid, [move.id], {
                    'move_dest_id': new_id,
