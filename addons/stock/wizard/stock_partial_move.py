...
class stock_partial_move_memory_out(osv.osv_memory):
    ...
    _columns = {
        ...
        'picking_qty' : fields.float("Quantity", required=True),       
    ...
    def onchange_qty(self, cr, uid, ids, move,pick):
        warning={}
        if move>pick:
            warning = {
                    'title': _('Quantite incorrect !'),
                    'message': _('Livraison en plus!!!') 
            } 
        qty=min(move,pick)
        return {'value':{'quantity': qty},'warning':warning}
