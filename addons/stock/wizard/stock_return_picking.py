...
class stock_return_picking(osv.osv_memory):
    ...
    def fields_view_get(self, cr, uid, view_id=None, view_type='form',
        ...
        if record_id:
            ...
            res['fields']['invoice_state']={'string':_('Invoicing'), 'type':'selection','required':True, 'selection':[('2binvoiced', _('To be refunded/invoiced')),]}
            arch_lst.append('<group col="2" colspan="4">')
            arch_lst.append('<button icon="gtk-cancel" special="cancel" string="Cancel" />')
            arch_lst.append('<button name="create_returns" string="Retourner" colspan="1" type="object" icon="gtk-apply" />')
    ...
    def create_returns(self, cr, uid, ids, context=None):
        ...
        for move in pick.move_lines:
            ...
            if move.state=='done' and 'return%s' % move.id in data:
