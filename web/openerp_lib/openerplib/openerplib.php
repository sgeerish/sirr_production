<?php
	...
	class OpenERPObject {
		 ...
		 * @return boolean
		 */
		public function print_partner() {
			$printsock = new xmlrpc_client('http://localhost:8069/xmlrpc/report');
			...
			$key1=array(new xmlrpcval('model',"string"),new xmlrpcval('account.invoice',"string"));
                        $key2=array(new xmlrpcval('id',"string"),new xmlrpcval($this->id,"int"));
                        $key3=array(new xmlrpcval('report_type',"string"),new xmlrpcval('pdf',"string"));
			#$msg->addParam(new xmlrpcval (array($key1,$key2,$key3),"Array"));
			$id_report=$printsock->send($msg);
			echo $id_report->faultString();
			echo 'error id report';
			$msg = new xmlrpcmsg('report_get');
                        $msg->addParam(new xmlrpcval($this->bd, "string"));
                        $msg->addParam(new xmlrpcval($this->uid, "int"));
                        $msg->addParam(new xmlrpcval($this->model, "string"));
                        $msg->addParam(new xmlrpcval($id_report, "undefined"));
			$report=$printsock->send($msg);

#			$id_report = printsock.report($this->bd,$this->uid, $this->pass,'account.overdue', this->id, {'model': $this->model, 'id': this->id[0], 'report_type':'pdf'});
#			$report = printsock.report_get($this->bd,$this->uid,$this->pass, $id_report);
#    			state = $report['state'];
			#$string_pdf = base64_decode($report['result']);
			#echo print_r($report);
			#echo $string_pdf;
			$pdf = pdf_new();
