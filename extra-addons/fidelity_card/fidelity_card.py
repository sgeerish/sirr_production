...
class fidelity_card_issue(osv.osv):
	...
	def validate(self,cr,uid,ids,args):
		for issue in self.browse(cr,uid,ids):
			self.write(cr,uid,issue.id,{'valid_user_id':uid,'state':'open'})
			self.pool.get('fidelity.card').write(cr,uid,issue.fidelity_card_id.id,{'state':'active','partner_id':issue.name.id})
		return True	
