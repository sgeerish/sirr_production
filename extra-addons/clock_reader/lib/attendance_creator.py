...
""" % (dt, empl_id, sql_action)
    ...
    def create_attendance(self, empl_id, dt, action, method, tolerance=60*5,
                          complete_attendance=False,ignore_signs=False):
#        assert isinstance(dt, tu.datetime)
        ...
        # Equal accion
	date_type=datetime.datetime.strptime('2010-01-01',"%Y-%m-%d")
	uni_type=type(''.decode('utf-8'))
	if type(dt)==uni_type:
        	dt=datetime.datetime.strptime(dt,"%Y-%m-%d %H:%M:%S")
        if ignore_signs and r and r[2]==action:
            if type(dt)==uni_type:
                dt=datetime.datetime.strptime(dt,"%Y-%m-%d %H:%M:%S")
            diff=r[1] + tu.timedelta(seconds=tolerance)
            if type(diff)==uni_type:
                diff=datetime.datetime.strptime(diff,"%Y-%m-%d %H:%M:%S")
            if diff> dt:
