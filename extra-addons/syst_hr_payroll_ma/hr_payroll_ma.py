...
class hr_payroll_ma(osv.osv):
    ...
    def action_bank_file_create(self,cr,uid,ids,context=None):
        import base64
        name='Salaire_'+time.strftime('%Y%m%d_%H%M%S')
        ...
        for payslip in salary.bulletin_line_ids:
            if payslip.employee_id.mode_reglement=='virement':
                c1=list('2')
                c2=list(' '*12)
                c3=list(' '*36)
        ...
        h2[0:8]=datetime.strftime(datetime.strptime(salary.x_date_transfer,'%Y-%m-%d'),'%d%m%Y')
        h3[0:6]=datetime.strftime(datetime.strptime(salary.x_date_transfer,'%Y-%m-%d'),'%d%m%y')
        h4[0:len(salary.partner_id.name[:36])]=salary.partner_id.name[:36]
        compte=salary.x_bank_id.acc_number.replace(' ','')
        rib=compte[len(compte)-2:]
        compte=compte[0:len(compte)-2]
...
class hr_payroll_ma_bulletin(osv.osv):
    ...
    def get_igr(self, cr, uid, ids, montant, cotisations):
        ...
        salaire_net_imposable=montant-cotisations
        ...
        ir_brute = (salaire_net_imposable  - (somme*12))*taux/100
    ...
    def compute_all_lines(self, cr, uid, ids, context={}):
        ...
        if salaire_base :
            ...
            salaire_base_line = {
                ...
                'rate_employee' : round((bulletin.working_days / days_work) * 100,2), 'subtotal_employee':round(salaire_base * (bulletin.working_days / days_work),2),'deductible' : False,
               }
        ...
        if taux_anciennete :
            anciennete_line = {
                'name' : 'g320 Prime danciennete', 'id_bulletin' : id_bulletin,'type' : 'brute',
            ...
            for cot in cotisations :
                ...
                if cot.plafond_tri:
                    ...
                    if int(mois)%3==0:
                        ...
                        old_bulletins=pool.get('hr.payroll_ma.bulletin').search(cr,uid,[('employee_id','=',bulletin.employee_id.id),('period_id','in',old_period)])
                        logger.notifyChannel('smtp', netsvc.LOG_INFO, old_bulletins)                        
                        old_bulletins=pool.get('hr.payroll_ma.bulletin').browse(cr,uid,old_bulletins)
                        ...
                        if total>plafond_trimestre:
                            ...
                            pool.get('hr.payroll_ma.bulletin.line').create(cr, uid, cotisation_line2)
