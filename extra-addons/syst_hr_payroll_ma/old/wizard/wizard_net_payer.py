...
fields = {    
    'fiscalyear_id':{'string': 'Exercice fiscal', 'type': 'many2one', 'relation': 'account.fiscalyear', 'required': False },
    ...
    'bank_id':{'string': 'Banque', 'type': 'many2one', 'relation': 'res.bank', 'required': True },
    #'employee_ids':{'string':'Employees', 'type':'many2many','relation':'hr.employee','required':True},
       }

class wizard_net(wizard.interface):
    ...
    states={
        ...
        'report':{
            'actions':[],
            'result':{'type':'print', 'report':'net.payer', 'state':'end'}
        }
    }
wizard_net('wizard.net.payer')
