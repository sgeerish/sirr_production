...
class hr_payroll_ma_bulletin(osv.osv):
    ...
    def onchange_employee_id(self, cr, uid, ids, employee_id,period_id):
        ...
        if period_id and employee_id :
            ...
            #        normal_hours = contract.monthly_hour_number
            print '>>>>>>>>>>>>>>>>>>>'
            print employee.contract_id[0]
            result = {'value': {
                            'employee_contract_id' : employee.contract_id,
                            #'employee_contract_id' : employee.contract_id.id,
                            'salaire_base' : employee.contract_id[0]['wage'],
                            'hour_base' : employee.contract_id[0]['hour_salary'],
                            'normal_hours' : employee.contract_id[0]['monthly_hour_number'],
    ...
    def get_igr(self, cr, uid, ids, montant, cotisations):
        ...
        print montant
        ir_brute = ((salaire_net_imposable- 250000) * 22 / 100)
        print '>>>>>>>>>>>>>'
        print cotisations
        print logement
...
#             'credit_account_id':dictionnaire['credit_account_id'][0],
    ...
    def compute_all_lines(self, cr, uid, ids, context={}):
        ...
        order by r.sequence
        '''
        cr.execute(sql, (0, 0, bulletin.period_id.date_start, bulletin.period_id.date_start))
 ...
 #               'name' : 'Impot sur le revenu', 'id_bulletin' : id_bulletin,'type' : 'cotisation','base' : res['salaire_net_imposable'], 'rate_employee' : res['taux'],
  #              'subtotal_employee':res['ir_net'],'credit_account_id': res['credit_account_id'], 'debit_account_id' : res['credit_account_id'],'deductible' : True,
   #             }
        ir_line = {
                'name' : 'Impot sur le revenu', 'id_bulletin' : id_bulletin,'type' : 'cotisation','base' : res['salaire_net_imposable'], 'rate_employee' : res['taux'],
                'subtotal_employee':res['ir_net'],'deductible' : True,
                }
        ...
        print round(salaire_net_a_payer,-2)
        arrondi=(round(salaire_net_a_payer,-2)-salaire_net_a_payer)
        salaire_net_a_payer_arrondi=round(salaire_net_a_payer,-2)
        #salaire_net_a_payer_arrondi+=arrondi
        print 'arrondi'
        print arrondi
        if  arrondi >1:
            arrondi=round(arrondi,2)
            print arrondi
        ...
        print 'net a payer arrondi'
        print salaire_net_a_payer_arrondi
        print salaire_net
        self.write(cr, uid, [bulletin.id], {   'salaire_brute' : salaire_brute,
                                               'salaire_brute_imposable':salaire_brute_imposable,
                                               'salaire_net':salaire_net_a_payer_arrondi,
                                               'salaire_net_a_payer':salaire_net_a_payer_arrondi,
...
class hr_ligne_rubrique(osv.osv):
    ...
    def onchange_calculate_price(self, cr, uid, ids, rubrique_id):
        rubrique = self.pool.get('hr.payroll_ma.rubrique').browse(cr, uid, rubrique_id)

        result = {'value': {
                            'montant' : rubrique.prix*rubrique.quantite,
                        }
                    }
