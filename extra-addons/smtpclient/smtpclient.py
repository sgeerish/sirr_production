...
class smtpclient(osv.osv):
    ...
    def send_ca(self,cr,uid,emailto,subject):
        sale_report=self.pool.get('sale.report')
        report=sale_report.search(cr,uid,[('date','<=',(datetime.today()+relativedelta(day=31)).strftime('%Y-%m-%d')),('date','>=',(datetime.today()-relativedelta(day=1)).strftime('%Y-%m-%d'))])
        report=sale_report.read(cr,uid,report,['x_brand_id','price_total'])
        brand_exist=[]
        locale.setlocale(locale.LC_ALL,'fr_FR.UTF-8')
        brand_obj=self.pool.get('product.brand')
        brand=brand_obj.search(cr,uid,[],order='x_seq')
        brand=brand_obj.browse(cr,uid,brand)
        brands={}
        logger.notifyChannel('reports', netsvc.LOG_WARNING, report)	
        #for r in report:
        #    brands[r['x_brand_id'][1]]+=r['price_total']
        for r in report:
            if r['x_brand_id'][0] not in brand:
                brand_exist.append(r['x_brand_id'][0])
        for b in brand:
            total=0
            brand_name=""
            for r in report:
                if r['x_brand_id'][0]==b:
                    brand_name=r['x_brand_id'][1]
                    total+=r['price_total']
            if brand_name=="":
                brand_name=brand_obj.browse(cr,uid,[b])[0].name
                
            brand_figures[brand_name]=total
                    
        cr.execute('select sum(price_total) from sale_report where EXTRACT(month FROM date)=EXTRACT(month FROM now()) and EXTRACT(year FROM date)=EXTRACT(year FROM now())')
        amount=float(cr.fetchone()[0] or 0)
        cr.execute('select sum(price_total) from sale_report where date=now()::date and EXTRACT(year FROM date)=EXTRACT(year FROM now())')
        jour=float(cr.fetchone()[0] or 0)
        date=time.strftime('%d-%m-%Y')
        amount=locale.format('%.2f', amount, True)
        jour=locale.format('%.2f', jour, True)
        body=date
        body+="<br/><table>"
        body+="<tr><td>CA du Jour</td><td align=\"right\">"+jour
        body+="</td></tr>"
        body+="<tr><td>CA du Mois</td><td align=\"right\">"+amount
        body+="</td></tr>"
        for key,value in brands.items():
            body+="<tr><td><b>"+key
            body+="</td><td align=\"right\">"
            body+=locale.format('%.2f', value, True)
            body+="</td></tr>"


        subject+="CA @"+date
        self.send_email(cr,uid,1,emailto,subject,body)
        return True

    def send_billetage(self,cr,uid,emailto):
        subject='Rapport Billetage'
        date=time.strftime('%Y-%m-%d')
        ids=self.pool.get('account.bank.statement').search(cr,uid,[('date','=',date),('journal_id.type', '=', 'cash')])
        rep=[]
        rep.append([('report.account.bank.statement.caisse'),ids])
        rep.append([('report.account.bank.statement.chq'),ids])
        self.send_email(cr,uid,1,emailto,subject,'',[],rep)
        return True

    def send_releve(self,cr,uid):
        cr.execute('select sum(price_total) from sale_report where EXTRACT(month FROM date)=EXTRACT(month FROM now())')
        amount=str(int(cr.fetchone()[0]))
        amount=','.join([amount[::-1][k:k+3][::-1] for k in xrange(len(amount)-1, -1, -3)])
        #amount="{:,}".format(amount)
        subject+=" CA : "+amount
        self.send_email(cr,uid,1,emailto,subject)
        return True        
	...
	vouchers=voucher_obj.search(cr,uid,[('date','=',date),('state','not in',['draft','cancel'])])
	...
	for voucher in vouchers:
		if voucher.type=='receipt':
			...
			payment_text+="<tr>"
			payment_text+="<td>"+voucher.name and voucher.name or ' ' +"</td>"
                        payment_text+="<td>"+locale.format('%.2f',voucher.amount,True) +"</td>"
			payment_text+="</tr>"

	net=receipt-payment
        body=date
        body+="<br/>"
        body+="CA du Jour : "+jour
        body+="<br/>"
        body+="CA du Mois : "+amount
        body+="<br/><br/>"
	body+="<b>Caisse</b><br/>"
	body+="Encaissements:<b> "+locale.format('%.2f', receipt, True)
	body+="</b><br/>Decaissements: <b>"+locale.format('%.2f', payment, True)
	body+="</b><br/>Net Caisse   : <b>"+locale.format('%.2f', net, True)

	body+="<br/><br/><b>Details de Decaissements</b>"
	body+="<table><tr><th>Details</th><th>Montant</th></tr>"
    ...
    def send_email(self, cr, uid, server_id, emailto, subject, body='', attachments=[], reports=[], ir_attach=[], charset='utf-8', headers={}, context={}):
        ...
        logger.notifyChannel('reports', netsvc.LOG_WARNING, reports)        
        for rpt in reports:
            logger.notifyChannel('rpt0', netsvc.LOG_WARNING, len(rpt))  
        #rpt=reports
            if len(rpt) == 3:
                rpt_file = createReport(cr, uid, rpt[0], rpt[1], rpt[2])
                logger.notifyChannel('rpt0', netsvc.LOG_WARNING, rpt[0])  
                logger.notifyChannel('rpt1', netsvc.LOG_WARNING, rpt[1])        
