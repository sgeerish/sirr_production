...
class Invoice(osv.osv):
    ...
    def get_total_quantity(self, cursor, user, ids, 
        ...
        return result
    def load_new_sales_price(self, cursor, user, ids,
            name=None, args=None, context=None):
        "Return the total quantity in an invoice"
        result = {}
        for invoice in self.browse(cursor, user, ids, context):
		for line in invoice.invoice_line:
			self.pool.get('account.invoice.line').write(cursor,user,line.id,{'new_sale_price':line.price_before})
        return True
    ...
    def set_product_cost(self, cr, uid, ids,
        ...
        self.write(cr,uid,ids,{'reviens_state':'calculated','x_date_revient':time.strftime('%Y-%m-%d')})
        return True
...
#        context.update({'to_date':self.browse(cursor, user, ids, context)[0].invoice_id.date_invoice})        
        for line in self.browse(cursor, user, ids, context):
            if line.product_id:
                ...
                result[line.id]=prod.browse(cursor,user,[line.product_id.id],context=context)[0].virtual_available
